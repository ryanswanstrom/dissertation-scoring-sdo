...
\label{sec:CRI}
    ...
    SDOs struggle to measure overall performance.  
    ...
    on current performance.  CRI is a collection of the following five 
    ...
    CRI is not meant to be comparative between
    ...
    A negative score indicates worse-than-expected performance and a 
    positive score indicates better-than-expected performance.   For
    example, a CRI score of 0.35 means the organization is 
    ...
    \subsection{ELEMENTS OF CRI}
        ...
        \subsubsection{QUALITY}
            ...
            \paragraph{QUALITY FORMULA}
                ...
                \begin{displaymath}
                   S_{1_i} = \left\{
                     \begin{array}{lr}
                       \text{where } f_i \geq d_i & :  \frac{f_i - d_i}{f_i} \cdot k  \\
                       \text{where } d_i > f_i  & : \frac{f_i-d_i }{\sigma_i^2} \cdot k
                     \end{array} \right.
                   \text{   , calculate quality score for each app $i$}
                \end{displaymath} 
                where
                \begin{itemize}
                    \item $S_{1_i}$ is the quality score for Application ID $i$
                    \item $k$ is the scaling factor to produce results in the range $[-k,k]$
        ...
        \subsubsection{AVAILABILITY}
            ...
            \paragraph{AVAILABILITY FORMULA}
                ...
                \begin{displaymath}
                   S_{2_i} = \left\{
                     \begin{array}{lr}
                       \text{where } A_{a_i} \leq A_{e_i} & : \frac{A_{a_i} - A_{e_i}}{A_{e_i}}\cdot k  \\
                       \text{where } A_{a_i} > A_{e_i}  & : \frac{A_{a_i} - A_{e_i} }{1-A_{e_i}}\cdot k
                     \end{array}
                   \right. \text{   , calculate availability score for each sys $i$}
                \end{displaymath}
                ...
                \begin{itemize}
                    ...
                    \item $k$ is the scaling factor to produce results in the range $[-k,k]$
                    \item $A_{a_i}$ is the actual availability for System ID $i$
                    \item $A_{e_i}$ is the expected availability for System ID $i$
                \end{itemize}
        ...
        \subsubsection{SATISFACTION}
            ...
            \paragraph{SATISFACTION FORMULA}
                ...
                \[
                    S_{3_i} = k \cdot \frac{ \sum^m_{j=1}(\frac{2a_{ij}-min-max}{max-min})}{m}
                \]
                
                \begin{itemize}
                    \item $S_{3_i}$ is the satisfaction score for Question ID $i$
                    \item $k$ is the scaling factor to produce results in the range $[-k,k]$
                    \item $a_{ij}$ is the answer to question $i$ for respondent $j$
                    \item $n$ is the number of questions
                    \item $m$ is the number of respondents
                    \item $min$ is the minimum score for a question
                    \item $max$ is the maximum score for a question
                \end{itemize}
        ...
        \subsubsection{SCHEDULE}  
            ...
            \paragraph{SCHEDULE FORMULA}
                ...
                where
                \begin{itemize}
                    \item $F_{a_i}$ is the actual finish date of project $i$
                    \item $F_{s_i}$ is the scheduled finish date of project $i$
                    \item $B_{s_i}$ is the scheduled beginning date of project $i$
                    \item $\Delta_i$ is the proportion the schedule was missed for project $i$
                \end{itemize}
            ...
            \paragraph{ALTERNATE APPROACH}
                ...
                \begin{displaymath}
                   S_{4_i} = \left\{
                     \begin{array}{lr}
                        \text{where } \Delta_i \geq 1 & : -1 \cdot k \\
                       \text{where }  \Delta_i \leq \bar{\Delta} & : \frac{\bar{\Delta} - \Delta_i}{\bar{\Delta}}   \cdot k  \\
                       \text{where } \Delta_i > \bar{\Delta} & : \frac{\bar{\Delta} - \Delta_i}{1 - \bar{\Delta}} \cdot k
                     \end{array}
                ...
                \begin{itemize}
                    ...
                    \item $F_{s_i}$ is the scheduled finish date of project $i$
                    \item $B_{s_i}$ is the scheduled beginning date of project $i$
                    \item $\Delta_i$ is the percent the schedule was missed
                ...
                \begin{displaymath}
                   S_{5_i} = \left\{
                     \begin{array}{lr}
                       ...
                       \text{where } R_{a_i} \leq R_{s_i} & : \frac{R_{a_i} - R_{s_i}}{R_{s_i}}\cdot k \\
                       \text{where } R_{a_i} > R_{s_i}  & : \frac{R_{a_i} - R_{s_i} }{b \cdot R_{s_i}}\cdot k
                     \end{array}
                   \right. \text{   , requirements score for project $i$}
                \end{displaymath}
                where
                \begin{itemize}
                    \item $S_{5_i}$ is the requirements score for Project ID $i$
                    \item $k$ is the scaling factor to produce results in the range $[-k,k]$
                    \item $R_{a_i}$ is the actual requirements completed for Project ID $i$
                    \item $R_{s_i}$ is the expected requirements completed for Project ID $i$
                    \item $b$ is the multiplier to determine the upper bound
                \end{itemize}
        ...
        \subsubsection{OVERALL CRI SCORE}
            ...
            as many elements as present.  This technique allows CRI to be implemented
