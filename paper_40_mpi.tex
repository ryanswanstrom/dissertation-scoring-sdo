...
\label{sec:MPI}
    ...
    on current performance.  MPI is a collection of the following 5 
    ...
    \subsection{ELEMENTS OF MPI}
        ...
        \subsubsection{SCHEDULE}  
            ...
            \paragraph{SCHEDULE FORMULA}
                ...
                the schedule score for MPI. Since all C.D.F. functions fall within the
                range $[0,1]$, the function needs to be shifted to center around 0, and then
                doubled to fill the desired range of $[-1,1]$. Thus the MPI schedule score
    ...
    \subsection{MPI COMPARED}
        ...
        \subsubsection{MPI VS. BALANCED SCORECARD}
            ...
            scorecard characteristics.
             ...
             \begin{longtable}{p{3cm}p{1cm}p{10cm}}
                ...
                Financial & No & MPI does not address financial as it is best suited 
                    for an organization that treats software development as a fixed,
                    budgeted expense. If the budget is fixed, MPI provides a number
                    to indicate the amount of value for that fixed budget.  \\
                \myrowcolour%
                Customer Focus & Yes & MPI includes a customer survey which is 
                    completely customer focused.\\
                Internal Process & Yes & MPI is highly focused on internal processes.
                    The MPI elements of schedule and requirement are completely
                    focused on how reality meets the expected process. MPI is
                    negatively impacted when internal processes are followed. \\
                \myrowcolour%
                Learning/Growth & No & MPI does not address this characteristic. \\
                
                \bottomrule
                
                \caption{BALANCED SCORECARD VERSUS MPI}
                \label{tab:bsc}
            \end{longtable}

        
        \subsubsection{MPI VS. PROJECT MANAGEMENT MEASUREMENT}
            ...
            as either the balanced scorecard or MPI. Actually, MPI incorporates
